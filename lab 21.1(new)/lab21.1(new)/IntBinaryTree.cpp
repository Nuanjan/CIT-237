...
void IntBinaryTree::displayInOrder(TreeNode *nodePtr,int depth) const
{
	if (nodePtr)
	{
...
void IntBinaryTree::displayNode(TreeNode *nodePtr, int depth) const
{
	// Display the values in this node.
	

	cout<<"("<<depth<<") node=" << nodePtr
		<< " value=" << setw(4) << nodePtr->value
		<< " left=" << setw(8);
	if (nodePtr->left)
		cout << nodePtr->left;
...
int IntBinaryTree::calculateSubTreeHeight(TreeNode *nodePtr) const
{
	...
	int lSubTreeHeight = calculateSubTreeHeight(nodePtr->left);
	int rSubTreeHeight = calculateSubTreeHeight(nodePtr->right);
...
void IntBinaryTree::displayTreeHeight() const
{
	...
	int treeHeight = 0;
	treeHeight = calculateSubTreeHeight(root);

	cout << "Height of tree = " << treeHeight-1;  
