...
void IntList:: printHead()const {
	ListNode *head;
	cout << "head=" << head;

}

void IntList::displayRecursive(ListNode *nodePtr) const {
	if (nodePtr == NULL)
	{
		cout << "\n";
		return;
	}
	cout << "Recursiving" << nodePtr->value << endl;
	displayRecursive(value->next);
}
	...
	/*void IntList::showReverse(ListNode *nodePtr) const {
		cout << "Enter showReverse, nodePtr=" << nodePtr << endl;
			if(nodePtr!=NULL)
			{ 
			
				showReverse(nodePtr->next);
				cout << "here is reverse" << nodePtr->value << endl;
	...
	int IntList:: findSum(ListNode *nodePtr) const{
		if (nodePtr != NULL)
			return nodePtr->value + findSum(nodePtr->next);
	}
	else
		return 0;
		}
